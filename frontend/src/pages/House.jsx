...
const House = () => {
  const { user, axios, backendUrl, navigate, calculateTotalPrice, handleBookingClick, isBookingOpen, setIsBookingOpen, houses } = useContext(AppContext)
  ...
  useEffect(() => {
    const checkHouseAvailability = async () => {
      if (!checkIn || !checkOut || !house) return
      try {
        const checkInStr = checkIn.toISOString().split('T')[0]
        const checkOutStr = checkOut.toISOString().split('T')[0]
        const { data } = await axios.get(`${backendUrl}/api/bookings/check-availability`, {
          params: { houseName: house.name, checkIn: checkInStr, checkOut: checkOutStr },
  ...
  }, [checkIn, checkOut, axios, backendUrl, house])
  ...
  const onSubmitHandler = async e => {
    ...
    if (availability === false) {
      toast.error('This house is already booked for selected dates')
      return
    }
    ...
    try {
      ...
      const totalPrice = calculateTotalPrice(checkIn, checkOut, house.price)
  ...
  return (
    <div className='min-h-screen p-6 text-sm md:text-base'>
      <div className='flex flex-col md:flex-row gap-10'>
        <img className='w-full md:w-1/2 md:h-140 rounded-xl shadow-lg' src={house.image?.[0]} alt={house.name} />
        ...
        <div className='flex flex-col gap-4 md:w-1/2 mr-10'>
          ...
          {house.bonus && <p className='mt-4 text-green-600 font-medium'>{house.bonus}</p>}

          {house.warning && <p className='mt-2 text-red-500 text-sm whitespace-pre-line'>{house.warning}</p>}

          <p className='mt-4 text-lg font-semibold'>Price: ${house.price} / night</p>
      ...
      {user && isBookingOpen && (
        <div className='fixed inset-0 flex items-center justify-center bg-black/50 z-50'>
          <div className='bg-white rounded-xl p-6 w-96 shadow-xl relative'>
            <h2 className='text-xl font-bold mb-4'>Booking {house.name}</h2>
            ...
            <form onSubmit={onSubmitHandler} className='flex flex-col gap-3'>
              ...
              {checkIn && checkOut && (
                <>
                  ...
                  <p className='mt-2 text-lg font-semibold text-green-500'>${calculateTotalPrice(checkIn, checkOut, house.price)}</p>
                </>
              )}
